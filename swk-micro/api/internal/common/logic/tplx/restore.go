...
func Restore(db, userID, jobID string, roleMess []*role.Role, domain, groupID, backupID, currentAppID, lang string, accessKeys []string) error {
	...
	go func() {
		...
		for _, ds := range dsData {
			...
			for i := 0; i < len(roleMess); i++ {
				var fields []string
				// 台账角色权限点
				var dsActions []*role.Action
				dsActionsMap := map[string]map[string]bool{}
				for _, pe := range peData {
					if pe.ObjectId == ds.GetDatastoreId() && pe.RoleName == roleMess[i].RoleName {
				...
				dsActions = append(dsActions, &role.Action{
					ObjectId:  dsRes.GetDatastoreId(),
					Fields:    fields,
					ActionMap: dsActionsMap[dsRes.GetDatastoreId()], // TODO 需要获取权限
				})
		...
		for _, r := range rpData {
			...
			for i := 0; i < len(roleMess); i++ {
				...
				for _, pe := range peData {
					if pe.ObjectId == r.ReportId && pe.RoleName == roleMess[i].RoleName {
						rpPe = pe.ActionMap
					}
				}
		...
		for i := 0; i < len(roleMess); i++ {
			var pmListN []*role.Permission
			for m := 0; m < len(pmList); m++ {
				if roleMess[i].RoleId == pmList[m].RoleId {
					pmListN = append(pmListN, pmList[m])
				}
			}

			// 角色更新参数
			var roleReq role.ModifyRoleRequest
			roleReq.RoleId = roleMess[i].RoleId
			roleReq.Database = db
			roleReq.Permissions = pmListN
