...
func (u *User) FindRelatedUsers(c *gin.Context) {
	...
	userService := user.NewUserService("manage", ct)
...
// ModifyUserMailStatus 更新用户邮箱状态
// @Router /update/users/mailstatus [put]
func (u *User) ModifyUserMailStatus(c *gin.Context) {
	...
	modifyID := appID + "modify"
	uploadID := appID + "upload"
	valUpload := store.Get(uploadID, false)

	var req user.ModifyUserMailStatusRequest
	...
	if len(valUpload) == 0 {
		store.Set(modifyID, "modify")
		go func() {
			// 创建任务
			jobx.CreateTask(task.AddRequest{
				JobId:        jobID,
				JobName:      "ModifyUserMailStatus",
				Origin:       "Modify User Mail Status",
				UserId:       userID,
				ShowProgress: false,
				Message:      i18n.Tr(lang, "job.J_014"),
				TaskType:     "Modify-User",
				Steps:        []string{"start", "mailverify", "end"},
				CurrentStep:  "start",
				Database:     db,
				AppId:        appID,
			})
			...
			jobx.ModifyTask(task.ModifyRequest{
				JobId:       jobID,
				Message:     i18n.Tr(lang, "message.M_001"),
				CurrentStep: "mailverify",
				Database:    db,
			}, userID)
			...
			_, err := userService.ModifyUserMailStatus(context.TODO(), &req, opss)
			if err != nil {
				store.Set(modifyID, "")
		...
		c.JSON(200, httpx.Response{
			Status:  0,
			Message: msg.GetMsg("ja-JP", msg.Info, msg.I003, fmt.Sprintf(httpx.Temp, ItemProcessName, ActionModifyUser)),
			Data: gin.H{
				"msg": i18n.Tr(lang, "message.M_002"),
			},
		})
	}
}
...
func (u *User) ModifyUser(c *gin.Context) {
	...
	if userInfo.GetNoticeEmail() != req.GetNoticeEmail() && req.GetNoticeEmail() != "" {
		...
		if len(val) == 0 {
			...
			httpx.GinHTTPError(c, ActionModifyUser, fmt.Errorf(i18n.Tr(lang, "error.E_015")))
			return
		}
	}
...
func (u *User) UploadUsers(c *gin.Context) {
	...
	mailverify := c.PostForm("mailverify")
	domain := sessionx.GetUserDomain(c)
	lang := sessionx.GetCurrentLanguage(c)
	userID := sessionx.GetAuthUserID(c)
	db := sessionx.GetUserCustomer(c)
	...
	modifyID := appID + "modify"
	uploadID := appID + "upload"
	valModify := store.Get(modifyID, false)
	...
	if len(valModify) == 0 {
		...
		go func() {
			...
			go func() {
				for _, data := range items {
					if mailverify == "2" {
						data.NoticeEmailStatus = "Verified"
					}
		...
		loggerx.InfoLog(c, ActionUploadUsers, loggerx.MsgProcessEnded)
		c.JSON(200, httpx.Response{
			Status:  0,
			Message: msg.GetMsg("ja-JP", msg.Info, msg.I004, fmt.Sprintf(httpx.Temp, ItemProcessName, ActionUploadUsers)),
			Data: gin.H{
				"msg": i18n.Tr(lang, "message.M_003"),
			},
		})
	}
...
func BuildAndValidate(p BVParam) (*user.UploadRequest, error) {
	...
	for i, h := range p.Header {
		...
		if h == "NoticeEmail" {
			...
			match, _ := regexp.MatchString(emailRegex, col)
			if !match {
				return nil, fmt.Errorf("メールフォーマットエラー")
			}
			if len(col) == 0 {
