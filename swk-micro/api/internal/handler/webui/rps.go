...
func (r *Prs) DownloadPrs(c *gin.Context) {
	...
	jobx.CreateTask(task.AddRequest{
		JobId:        jobID,
		JobName:      "datastore file download(本金返还预计表)",
	...
	go func() {
		...
		var headers [][]string
		// 添加ID
		header = append(header, "租赁公司")
		header = append(header, "契约番号")
		header = append(header, "类别")
		header = append(header, "1年以内")
		header = append(header, "超过1年")
		header = append(header, "超过1年-2年以内")
		header = append(header, "超过2年-3年以内")
		header = append(header, "超过3年-4年以内")
		header = append(header, "超过4年-5年以内")
		header = append(header, "超过5年")
				...
				for i := startpos; i < endpos; i++ {
					...
					for _, v := range lres {
						if v.Leasekaisha == kres[i].Leasekaisha && v.Keiyakuno == kres[i].Keiyakuno {
							...
							// 1年以内
							if paymentym.Before(syoriym.AddDate(0, 12, 0)) {
								prs.Repayment1 += v.Repayment
							}
							// 超过1年
							if paymentym.Equal(syoriym.AddDate(0, 12, 0)) || (paymentym.After(syoriym.AddDate(0, 12, 0)) && paymentym.Before(syoriym.AddDate(0, 60, 0))) {
								prs.Repayment2 += v.Repayment
							}
							// 超过1年-2年以内
							if paymentym.Equal(syoriym.AddDate(0, 12, 0)) || (paymentym.After(syoriym.AddDate(0, 12, 0)) && paymentym.Before(syoriym.AddDate(0, 24, 0))) {
								prs.Repayment21 += v.Repayment
							}
							// 超过2年-3年以内
							if paymentym.Equal(syoriym.AddDate(0, 24, 0)) || (paymentym.After(syoriym.AddDate(0, 24, 0)) && paymentym.Before(syoriym.AddDate(0, 36, 0))) {
								prs.Repayment22 += v.Repayment
							}
							// 超过3年-4年以内
							if paymentym.Equal(syoriym.AddDate(0, 36, 0)) || (paymentym.After(syoriym.AddDate(0, 36, 0)) && paymentym.Before(syoriym.AddDate(0, 48, 0))) {
								prs.Repayment23 += v.Repayment
							}
							// 超过4年-5年以内
							if paymentym.Equal(syoriym.AddDate(0, 48, 0)) || (paymentym.After(syoriym.AddDate(0, 48, 0)) && paymentym.Before(syoriym.AddDate(0, 60, 0))) {
								prs.Repayment24 += v.Repayment
							}
							// 超过5年
							if paymentym.Equal(syoriym.AddDate(0, 60, 0)) || paymentym.After(syoriym.AddDate(0, 60, 0)) {
								prs.Repayment5 += v.Repayment
							}
						}
					}
					// 编辑集计后元本
					...
					itemData2 = append(itemData2, strconv.FormatFloat(prs.Repayment1, 'f', -1, 64))
					itemData2 = append(itemData2, strconv.FormatFloat(prs.Repayment2, 'f', -1, 64))
					itemData2 = append(itemData2, strconv.FormatFloat(prs.Repayment21, 'f', -1, 64))
					itemData2 = append(itemData2, strconv.FormatFloat(prs.Repayment22, 'f', -1, 64))
					itemData2 = append(itemData2, strconv.FormatFloat(prs.Repayment23, 'f', -1, 64))
					itemData2 = append(itemData2, strconv.FormatFloat(prs.Repayment24, 'f', -1, 64))
					itemData2 = append(itemData2, strconv.FormatFloat(prs.Repayment5, 'f', -1, 64))
					// 添加行
