...
// FindUsers 查找多个用户记录
func FindUsers(ctx context.Context, db, userName, email, group, app, role, status, domain, invalidatedIn, errorCount string) (u []User, err error) {
	client := database.New()
	c := client.Database(database.GetDBName(db)).Collection(UsersCollection)
	...
	// 邮箱验证状态不为空
	if status != "" {
		query["notice_email_status"] = status
	}
...
func ModifyUserMailStatus(ctx context.Context, db string, userIDlist []string, writer string) (err error) {
	client := database.New()
	c := client.Database(database.GetDBName(db)).Collection(UsersCollection)
	/* ctx, cancel := context.WithTimeout(context.Background(), 120*time.Second)
	defer cancel() */
	...
	for _, id := range userIDlist {
		...
		change["updated_by"] = writer

		// 更新邮箱状态
		change["notice_email_status"] = "Verified"
