...
type LRParam struct {
	...
	Item                    *item.Item        `json:"item_map" bson:"item_map"`                               // 数据
	seq                     string
}
...
func compute(db, appID, userID string, p LRParam) (result *ComputeResult, err error) {
	...
	// 租赁满了年月日检查
	checkErr = expireymdCheck(p.Leasestymd.Format("2006-01-02"), lastPayYmd, p.Leasekikan, p.ExtentionOption)
	if checkErr != nil {
		loggerx.ErrorLog("compute", checkErr.Error())
		return nil, checkErr
	}
	...
	// **********利息情报算出**********
	leases, err := getLeaseData(payments, leasestymd, leasestymd, principalAmount, rishiritsu)
	if err != nil {
		loggerx.ErrorLog("compute", err.Error())
		return nil, err
	}
	...
	// 偿还情报算出处理
	repays, err := getRepayData(leasestymd, genkakikan, residualValue, boka, appID, p.BeginMonth, db)
	if err != nil {
		loggerx.ErrorLog("compute", err.Error())
		return nil, err
	}
...
func buildPayData(path string) (map[string]PayData, error) {
	...
	for {
		...
		if index > 1 {
			...
			paymentymd := cast.ToTime(row[indexMap["paymentymd"]])
			...
			pay := Payment{
				...
				Paymentymd:           paymentymd.Format("2006-01-02"),
