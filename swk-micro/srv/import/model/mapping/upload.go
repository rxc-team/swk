					...
					if value != "" {
						...
						if stringx.SpecialCheck(value, p.Special) {
							...
							for _, f := range p.Fields {
								if mp.FromKey == f.FieldId {
									if model.CharCount(value) < int(f.MinLength) {
										errorList = append(errorList, fmt.Sprintf("[%s]=>フィールドの長さが%s桁以上でなければなりません。", value, cast.ToString(f.MinLength)))
										break Loop
									}
									if model.CharCount(value) > int(f.MaxLength) {
										errorList = append(errorList, fmt.Sprintf("[%s]=>フィールドの長さが%s桁以下でなければなりません。", value, cast.ToString(f.MaxLength)))
										break Loop
									}
								}
							}
		...
		case "number":
			if mp.PrimaryKey {
				...
				if value, ok := rowItems[mp.ToKey]; ok {
					...
					if value != "" {
						// 判断是否是数字类型
						numValue, e := strconv.ParseFloat(value, 64)
						if e != nil {
							...
							for _, f := range p.Fields {
								if mp.FromKey == f.FieldId {
									if numValue < float64(f.MinValue) {
										errorList = append(errorList, fmt.Sprintf("[%s]=>フィールドの大きさが%s以上でなければなりません。", value, cast.ToString(f.MinValue)))
										break Loop
									}
									if numValue > float64(f.MaxValue) {
										errorList = append(errorList, fmt.Sprintf("[%s]=>フィールドの大きさが%s以下でなければなりません。", value, cast.ToString(f.MaxValue)))
										break Loop
									}

									parts := strings.Split(value, ".")
									if len(parts) == 2 {
										if model.CharCount(parts[1]) > int(f.Precision) {
											errorList = append(errorList, fmt.Sprintf("[%s]=>フィールドの小数点以下の桁数%s以下でなければなりません。", value, cast.ToString(f.Precision)))
											break Loop
										}
									}
								}
						...
						if mp.IsRequired {
							...
							if value != "" {
								...
								if e != nil {
									...
									for _, f := range p.Fields {
										if mp.FromKey == f.FieldId {
											if nv < float64(f.MinValue) {
												errorList = append(errorList, fmt.Sprintf("[%s]=>フィールドの大きさが%s以上でなければなりません。", value, cast.ToString(f.MinValue)))
												break Loop
											}
											if nv > float64(f.MaxValue) {
												errorList = append(errorList, fmt.Sprintf("[%s]=>フィールドの大きさが%s以下でなければなりません。", value, cast.ToString(f.MaxValue)))
												break Loop
											}
										}
