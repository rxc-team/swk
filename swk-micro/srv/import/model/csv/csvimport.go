...
func Import(base Params, file FileParams) {
	...
	// 读取文件
	fs, err := os.Open(file.FilePath)
	if err != nil {
		loggerx.ErrorLog("readCsvFileAndImport", err.Error())
		path := filex.WriteAndSaveFile(cparam.domain, cparam.appID, []string{err.Error()})
	...
	items, _, err := readFile(cparam, file.FilePath, total)
	if err != nil {
		loggerx.ErrorLog("readCsvFileAndImport", err.Error())
		return
	}
	fmt.Println("读取成功", items)
...
// readFile 读取文件
func readFile(p checkParam, filePath string, total int) (data []item.ImportData, wf string, e error) {
	...
	langI.Common = (*item.Common)(p.langData.Common)
	for key, value := range p.langData.Apps {
	...
	for {
		...
		if index > 1 {
			...
			// 数据验证并生成数据
			_, checkErrors := checkAndBuildItems(p, langI, uMap)
		...
		if (index-1)%500 == 0 && len(errorList) == 0 {
			// 返回消息
		...
		if (index-1) == total && len(errorList) == 0 {

			// 返回消息
			result := make(map[string]interface{})

			if p.action == "insert" {
				result["inserted"] = index - 1
				result["updated"] = 0
			}

			if p.action == "update" {
				result["updated"] = index - 1
				result["inserted"] = 0
			}
...
func checkAndBuildItems(p checkParam, langI item.Language, uMap map[string]string) (list *item.ListItems, result []*item.Error) {
		...
		if fieldInfo.FieldType == "number" {
			...
			if len(col) == 0 {
				...
				if err != nil {
					...
					} else if len(parts) == 2 && model.CharCount(parts[1]) > int(fieldInfo.Precision) {
						checkDataExistError = append(checkDataExistError, &item.Error{CurrentLine: line, FieldId: fieldInfo.FieldId,
							ErrorMsg: "[" + col + "]=>" + "フィールドの小数点以下の桁数" + cast.ToString(fieldInfo.Precision) + "以下でなければなりません。",
						})
					} else {
						cols[field] = &item.Value{
							DataType: fieldInfo.FieldType,
	...
	var req item.ImportCSVRequest
	req.Items = rowMap
	req.DatastoreId = p.datastoreID
	req.DB = p.db
	req.Action = p.action
	req.UserId = p.userID
	req.EmptyChange = p.emptyChange
	req.AppId = p.appID
	req.Owners = p.owners
	req.LangCd = p.lang
	req.Domain = p.domain
	...
	ress, _ := itemService.ImportCSVItem(context.Background(), &req)
