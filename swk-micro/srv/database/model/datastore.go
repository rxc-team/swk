...
type (
	...
	Datastore struct {
		...
		FieldWidth          string             `json:"field_width" bson:"field_width"`
	...
	MappingConf struct {
		...
		DownloadEncoding string         `json:"download_encoding" bson:"download_encoding"`
...
func (m *MappingConf) ToProto() *datastore.MappingConf {
	...
	return &datastore.MappingConf{
		...
		DownloadEncoding: m.DownloadEncoding,
		MappingRule:      rules,
	}
}
...
func ModifyDatastore(db string, ds *ModifyReq) (err error) {
	...
	// FieldWidth的场合
	change["field_width"] = ds.FieldWidth
...
func ModifyMapping(db, appID, datastoreID string, mp MappingConf) (err error) {
	...
	change := bson.M{
		"$set": bson.M{
			...
			"mappings.$.download_encoding": mp.DownloadEncoding,
