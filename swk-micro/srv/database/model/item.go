...
import (
	...
	"rxcsoft.cn/pit3/srv/database/common/jobx"
...
func searchBefore(ctx context.Context, c *mongo.Collection, db string, query bson.M, params ItemsParam) (bson.D, error) {
	...
	// 编辑 match 检索条件
	buildMatchAndSort(params.ConditionList, params.ConditionType, query, &indexKeys, indexMap, params.Timezone)
...
func buildMatchAndSort(conditionList []*Condition, conditionType string, query bson.M, indexKeys *[]string, indexMap bson.M, Timezone string) {
				...
				if condition.IsDynamic {
					...
					case "datetime":
						...
						if condition.ConditionType == "1" {
							...
							if condition.Operator == ">" {
								value := getTime(condition.SearchValue)
								timeValue := value.Format("2006-01-02 15:04:05")
								loc, _ := time.LoadLocation(Timezone)
								datetimezone, _ := time.ParseInLocation("2006-01-02 15:04:05", timeValue, loc)
								utcTimeValue := datetimezone.UTC()
								fullTime := utcTimeValue.Add(time.Hour * 24)
								and = append(and, bson.M{
									condition.FieldID: bson.M{"$gte": fullTime},
								})
							} else if condition.Operator == ">=" {
								...
								utcTimeValue := datetimezone.UTC()
								and = append(and, bson.M{
									condition.FieldID: bson.M{"$gt": utcTimeValue},
								})
							} else if condition.Operator == "<" {
								// 小于的场合
								value := getTime(condition.SearchValue)
								...
								utcTimeValue := datetimezone.UTC()
								and = append(and, bson.M{
									condition.FieldID: bson.M{"$lt": utcTimeValue},
								})
							} else if condition.Operator == "<=" {
								// 小于等于的场合
								value := getTime(condition.SearchValue)
					...
					case "datetime":
						...
						if condition.ConditionType == "1" {
							...
							if condition.Operator == ">" {
								...
								utcTimeValue := datetimezone.UTC()
								q := bson.M{
									condition.FieldID: bson.M{"$gte": utcTimeValue},
								}
								or = append(or, q)
							} else if condition.Operator == "<" {
								// 小于的场合
...
func ResetInventoryItems(db, userID, appID, jobID string) (err error) {
	client := database.New()
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Minute)
	defer cancel()
	jobx.ModifyTask(task.ModifyRequest{
		JobId:       jobID,
		Message:     "棚卸ステータスのリセット開始",
		CurrentStep: "deal-data",
		Database:    db,
	}, userID)
